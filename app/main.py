...
# --- Вкладка Обучение ---
with tab4:
    # 1. Сохраняем ваш блок JSON (Казахстанские разработки)
    ...
    # 2. Создаем вложенную навигацию для обучения
    inner_tabs = st.tabs([
        "📖 Интерактивное пособие", 
        "🎥 Видеолекции", 
        "🧪 Лабораторный практикум", 
        "🔬 Кейсы для магистрантов"
    ])

    # --- ПОДВКЛАДКА 1: ПОСОБИЕ И ЧАТ ---
    with inner_tabs[0]:
        col1, col2 = st.columns([2, 1])
        with col1:
            st.subheader("Теоретический материал")
            # Сюда будем подгружать главы 4-5 из вашего файла
            st.info("Выберите главу в боковой панели, чтобы начать чтение.")
        with col2:
            st.subheader("ИИ-Тьютор")
            st.write("Задайте вопрос по тексту пособия:")
            st.text_input("Ваш вопрос:", key="qsar_chat")

    # --- ПОДВКЛАДКА 2: ВИДЕО И ВОПРОСЫ ---
    with inner_tabs[1]:
        st.header("Курс лекций")
        st.video("https://www.youtube.com/watch?v=dQw4w9WgXcQ")
        st.markdown("### Проверка понимания")
        # Сюда переносим ваши тесты
        st.radio("Как изменится липофильность (LogP) при добавлении -OH группы?", 
                 ["Увеличится", "Уменьшится", "Не изменится"], key="video_test")

    # --- ПОДВКЛАДКА 3: ЛАБОРАТОРНЫЕ ---
    with inner_tabs[2]:
        st.header("Практические задания для студентов")
        # Сюда пойдут инструкции из глав 6 и 8 вашего файла
        st.write("Выберите лабораторную работу из списка...")

    # --- ПОДВКЛАДКА 4: КЕЙСЫ ДЛЯ МАГИСТРАНТОВ ---
    with inner_tabs[3]:
        st.header("Исследовательские кейсы (Advanced)")
        # Сюда добавим расширенную аналитику и сложные задачи
        st.write("Разбор научных публикаций и прогнозирование сложных систем.")
